...
from pyzdata import Config, Interval, __version__
from pyzdata.demo_client import DemoPyZData
...
def get_client() -> DemoPyZData:
    # Always return a demo client in demo mode
    if "client" not in st.session_state:
        st.session_state["client"] = DemoPyZData()
...
def render_sidebar() -> None:
    st.sidebar.markdown("# 📊 PyZData Demo")
    st.sidebar.caption(f"v{__version__} • Demo Mode • No Login Required")
    st.sidebar.divider()
    
    st.sidebar.success("🎉 Demo Mode Active")
    st.sidebar.info(
        "This is a demo version that generates realistic sample data. "
        "No Zerodha account needed!"
    )
    st.sidebar.divider()
    
    with st.sidebar.expander("ℹ️  About Demo Mode"):
        st.markdown("""
**What this demo provides:**
- Realistic OHLCV data for popular Indian stocks
- All time intervals (1min to daily)
- Sample data for the last 5 years
- Full export functionality

**Limitations:**
- Data is generated, not real market data
- Only popular stocks are available
...
def render_welcome() -> None:
    st.markdown("""
## Welcome to PyZData Demo
### Explore Indian stock market data without an account
""")
...
<div class="info-card">
<b> Demo Data</b><br><br>
Get realistic sample data for popular Indian stocks and indices. No real account needed.
</div>
...
Choose from last week all the way to 5 years of demo data, or pick custom dates.
</div>
""", unsafe_allow_html=True)
    with c3:
        st.markdown("""
<div class="info-card">
<b> Export ready</b><br><br>
Download your demo data instantly as CSV or Excel. Perfect for testing and learning.
</div>
""", unsafe_allow_html=True)
    ...
    with s1:
        st.markdown("**Step 1 - Choose a stock**\n\nSelect from popular Indian stocks or type any symbol.")
    with s2:
        st.markdown("**Step 2 - Set time range**\n\nPick a date range and data frequency for your analysis.")
    with s3:
        st.markdown("**Step 3 - Download**\n\nGenerate realistic demo data and export to your preferred format.")

    st.divider()
    st.success(" **Ready to start! Select a stock from the options below.**")
...
def _execute_download(
    ...
    # Generate demo data
    with st.spinner(f"Generating demo data for {symbol} on {exchange} ..."):
        try:
            # Convert date objects to datetime for demo client
            ...
            st.error(f"Error generating demo data: {exc}")
            return

    # Show progress (demo data generation is fast, but we'll show the UI)
    progress_bar = st.progress(0, text=f"Generating **{symbol}** demo data ...")
    progress_bar.progress(0.5, text=f"Creating realistic price movements ...")
    progress_bar.progress(1.0, text="Demo data ready!")
